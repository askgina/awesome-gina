...
- For recipe submissions, include a `Quick Copy Prompt (Ask Gina)` section with a fenced `text` code block that begins with `promptText:`.
...
- quick-copy `promptText` block (for recipe)
...
- Recipe submissions include a noob-friendly `Quick Copy Prompt (Ask Gina)` `promptText` block.
